    ...
    const app = {
                ...
                return `
                  <li class="content__songslist-iner">
                        ...
                        <div class="content__songslist-thumneil">
                            <img src="${song.img}" class="content__songslist-img" />
                        </div>
        ...
        handleEvent: function() {
            ...
            $.each(iconListMore, function(index, iconMore) {
                ...
                //click để ẩn
            $(document).click((e) => {
                if (!iconListMore.is(e.target) && iconListMore.has(e.target).length === 0) {
                    listMore.hide();
                }
            })
            ...
            $(audio).on("timeupdate", function() {
                    if ($(this).prop("duration")) {
                        const progress = Math.floor(($(this).prop("currentTime")) / ($(audio).prop("duration")) * 100)
            ...
            $(".progress").on("change", function() {
                const seekTime = $(".progress").val() / 100 * ($(audio).prop("duration"))
        ...
        nextSong: function() {
            app.currentIndex = app.currentIndex + 1 >= app.songs.length ? 0 : app.currentIndex + 1
            app.loadCurrentSong();

        },
        prevSong: function() {
            app.currentIndex = app.currentIndex - 1 < 0 ? app.songs.length - 1 : app.currentIndex - 1
            app.loadCurrentSong();
        },
        randomSong: function() {
            let newIndex;
